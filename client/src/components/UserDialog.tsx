...
const UserDialog = ({ user, updateUser, handleLogin, handleRegister }: Props) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center outline outline-8 mx-auto py-9 w-3/12 h-4/6 gap-9 rounded-3xl bg-white">
      {!user && (
        <>
          <h1 className="text-3xl mb-3.5">Welcome to the Streaks App!</h1>
        ...
        <>
          ...
          <div className="flex flex-col gap-2 items-center">
            ...
            <div className="flex flex-row font-bold text-5xl">
              {user.coins} <PiCoinVerticalFill className="fill-yellow-500" />
            </div>
          </div>
          ...
          >
            {disableButton ? "Come Back Tomorrow!" : "Get Coins"}
          </button>
        </>
      )}
    </div>
